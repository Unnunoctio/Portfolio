...
import { AboutMe, ContactMe, Resume } from '../views'

export const UserMenu = (): React.ReactNode => {
  const menuItems = [
    { title: 'About me', code: 'about-me' },
    { title: 'Resume', code: 'resume' },
    { title: 'Contact me', code: 'contact-me' }
  ]

  const [view, setView] = useState<string | null>(null)

  const onChangeView = (code: string): void => setView(code)
  const onResetView = (): void => setView(null)
  ...
  return (
    <>
              ...
              <div className='flex flex-col gap-3 p-5'>
                {menuItems.map((item, index) => (
                  <Menu.Item as='div' key={index}>
                    <button onClick={() => onChangeView(item.code)} className='px-3 py-1 w-full rounded-md text-lg text-start text-text-secondary cursor-pointer transition-colors hover:text-text-hover hover:bg-component-hover'>
                      {item.title}
                    </button>
                  </Menu.Item>
                ))}
      ...
      {view === 'resume' && <Resume onResetView={onResetView} />}
